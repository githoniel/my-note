...
site1: x -> xz -> (site3优先，O3优先, O3的因果01被传递优先) -> xzy
...
`GROVE`解决了这个问题。依旧用之前的例子

site3: y -> zy -> zxy

第一步: `O2' = IT(O3, O2) = insert(y, 2)`, 得到 `zy`, 此时 `EC = O3 => O2'`
第二部: 由于 `EC = O3 => O1 => O2`, 需要撤销`O2'`，然后执行`O1`，执行`O2'' = IT(O2', O1)`, 得到 `z -> xz -> xzy`，此时`EC = 03 => 01 =>O2''`

## `Jupiter`实现

`Jupiter`协同系统是施乐帕克研究中心(Xerox PARC)研发。由于`Jupiter`系统已经有了一个中心服务器用于保存状态(比如白板、文档等等)，所以使用中心服务器用于支持一致性算法是很自然的。这个算法。`Jupiter`一致性保持算法是从`dOPT`算法衍生而来的。最有意思部分是，它是基于一个多客户端+一个中心服务端的环境下优化`dOPT`算法。

`Jupiter`和`GROVE`类似，共享文档在每个站点都有一份备份，不同点在于在中心服务器也保存了一份文档备份，而且通讯只发生在`service-client`端。当本地站点产生`OP`的时候，会在本地立刻执行，之后传播到中心服务器。中心服务器在需要的时候首先转换`OP`，在中心服务器执行后，广播转换后的`OP`。站点收到`OP`后也需要转换`OP`后再执行。这种星形拓扑结构消除了对确保`因果关系`的担忧，大幅度简化了`OP`控制算法。

`Jupiter`采用了一个二维状态空间图，替代了`dOPT`算法中的线性`LOG/HB`，用于记录所有可能的操作路径。`Jupiter`算法确保任意一对涉及转换的操作都必须来源于状态空间图中的同一个起点，非常类似于`REDUCE`实现中的`GOT`算法的`context等价`。因此，`Jupiter`算法可以在只允许客户端-服务端交流的条件下，修正`dOPT`算法。

## `adOPTed`算法

原则和`GROVE`类似，使用分布式结构, 不同的是, `adOPTed`算法额外要求了一个原则。

给定2个操作`Oa`, `Ob`, Oa' = T(Oa, Ob), Ob' = T(Ob, Oa)

- 转换原则1(TP1): `Oa * Ob' = Ob * Oa'`
- 转换原则1(TP2): 对于任意操作`O`，`T(T(O, Oa), Ob') = T(T(O, Ob), Oa')`，也就是

`T(T(O, Oa), Ob' = T(Ob, Oa)) = T(T(O, Ob), Oa' = T(Oa, Ob))`

`TP1`是`dOPT`算法和`Jupiter`算法也同样要求的，但是`TP2`是新的要求。这两个原则可以用图来表示，合称`Interaction Model`

![fig-5](img/fig-5.png)

有论文可以证明，`TP1`和`TP2`是`P2P`环境就下实现一致性的必要条件。

## `GOTO`算法

基于`GOT优化`， 优化了上面`case3`的算法，上面的`GOT`算法是选择全部`ET`完成后`IT`，导致左侧回退太多，`GOTO`则采用了`ET`, `IT`交替进行的方法，简化了算法，虽然还是很复杂。

## 总结

### 主要成就

识别到三大不一致问题 - 发散，违反因果关系，违法编辑意图，特别是不可序列化的违反编辑意图的问题和可以序列化的发散问题的区分。

此外转换的context、ET、IT转换函数、转换前置后置条件等等也非常重要。

### 遗留问题和未来的方向

基于`OP`的转换，其正确性依赖于同时满足转换的前置和后置条件。问题是算法无法做到通用。如何设计一套通用算法是一个方向。如何保证算法满足TP1,TP2还是未知的。